...
class _FriendFeedState extends State<FriendFeed> {
  ...
  Future<void> _pageSetup() async {
    ...
    if (currentUser != null) {
      // Get the current user's friends from Firestore
      final userDoc = await FirebaseFirestore.instance.collection('users').doc(currentUser.uid).get();
      if (userDoc.exists) {
        final friendsList = List<String>.from(userDoc.data()?['friends'] ?? []);

        // Fetch creator_ids for the friends
        List<String> creatorIds = [];
        for (String friendUsername in friendsList) {
          final friendDoc = await FirebaseFirestore.instance.collection('users').where('username', isEqualTo: friendUsername).get();
          if (friendDoc.docs.isNotEmpty) {
            creatorIds.add(friendDoc.docs.first.data()['user_id']);
          }
        }

        // Fetch geocaches created by friends
        if (creatorIds.isNotEmpty) {
          final geocacheQuery = await FirebaseFirestore.instance.collection('geocaches')
              .where('creator_id', whereIn: creatorIds)
              .get();

          List<Map<String, dynamic>> geocaches = geocacheQuery.docs.map((doc) => doc.data() as Map<String, dynamic>).toList();
          setState(() {
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () async {
                              try {
                                // Assuming `FirebaseAuth` is used for the current user
                                final currentUser = FirebaseAuth.instance.currentUser;
                                final creatorId = geocache['creator_id'];

                                if (currentUser != null && creatorId != null) {
                                  final userId = currentUser.uid;

                                  // Add the creator's ID to the current user's list in Firestore
                                  await FirebaseFirestore.instance
                                      .collection('users')
                                      .doc(userId)
                                      .update({
                                    'friends': FieldValue.arrayUnion([creatorId])
                                  });

                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('Added creator to your list!'),
                                      duration: const Duration(seconds: 2),
                                    ),
                                  );
                                }
                              } catch (e) {
                                print('Error adding creator: $e');
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text('Failed to add creator!'),
                                    duration: const Duration(seconds: 2),
                                  ),
                                );
                              }
